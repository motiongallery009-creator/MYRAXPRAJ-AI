  ...
  wss.on("connection", async (clientWs) => {
    ...
    try {
      ...
      const baseInstructions = 
        ...
        "     4. To SUBMIT the search (mandatory, do not stop after just typing): call 'readScreenContent' again and look specifically near the BOTTOM of the screen (where the on-screen keyboard is) for a 'Search' or 'Go' action key, then 'tapOnScreen' that exact element. IMPORTANT: NEVER tap any element whose text or description contains the words 'voice', 'microphone', or 'mic' (e.g. 'Search with your voice', 'Voice Search') - these open voice input instead of submitting the typed text, which is wrong. If you cannot clearly identify the keyboard's bottom-right action key, try 'tapOnScreen' with text='Go' or text='Enter' as a fallback, but avoid re-tapping the top search bar/icon again since that may reopen voice search instead of submitting.\n" +

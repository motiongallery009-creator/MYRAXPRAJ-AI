...
export class MyraaAudioSession {
  ...
  public async connect() {
    ...
    try {
      // 1. Establish custom WebSocket server bridge using dynamic configuration
      const wsUrl = getWebSocketUrl();
      console.log("[Myraa] Establishing holographic link via:", wsUrl);
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          if (typeof navigator === "undefined" || !navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== "function") {
            throw new Error("Microphone API (getUserMedia) is not supported or accessible (requires secure HTTPS context or WebView permissions).");
          }
